...
export function ExecutionProgress({ simulationId }: ExecutionProgressProps) {
  ...
  if (data.isActive && data.queuePosition !== -1) {
    return (
      <Box className={classes.noLogsContainer}>
        <Loader />
        <Text size="lg">Your simulation is in the queue (position: {data.queuePosition})</Text>
        <RefetchTime />
      </Box>
    );
  }
  ...
  if (!data.isStored) {
    return (
      <Box className={classes.noLogsContainer}>
        ...
        <Text size="lg">This simulation is not stored anymore. Execution info is unavailable.</Text>
      </Box>
    );
  }
